defmodule DroneFeed.Accounts.UserToken do
  use Ecto.Schema
  import Ecto.Query
  alias DroneFeed.Accounts.UserToken
  ...
  @doc """
  Builds a confirmation token lookup query (valid for #{@confirm_validity_in_days} days).
  """
  def verify_email_token_query(token, "confirm") do
    case Base.url_decode64(token, padding: false) do
      {:ok, decoded_token} ->
        hashed_token = :crypto.hash(@hash_algorithm, decoded_token)
